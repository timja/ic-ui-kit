...
export class DateInput {
  ...
  // TODO: Update styling with 'fit-to-value' when null to .value
  ...
  private handleInput = (event: Event) => {
    ...
    if (input !== this.yearInputEl) {
      ...
      if (
        ...
        if (
          ...
          // input.value = `0${inputEvent.data}`; // Is this required if blur does the autocomplete
    ...
    this.notifyScreenReader(input?.value);
  };
  ...
  private handleUpDownArrowKeyPress = (
    ...
    if (input?.value) {
      ...
      if (event.key === "ArrowDown") {
        event.preventDefault();
        input.value = maxValue.toString();
        this.notifyScreenReader(maxValue.toString());
      } else if (event.key === "ArrowUp") {
        event.preventDefault();
        input.value = minValue.toString();
        this.notifyScreenReader(minValue.toString());
      }
    }
  };
  ...
  private setValidationMessage = () => {
    ...
    if (!isEmptyString(this.day)) {
      if (+this.day > 31) {
        isValidDay = false;
        // this.dayInputEl.setAttribute("aria-invalid", "true");
      }
    } else {
      isValidDay = true;
    }
  ...
  private notifyScreenReader = (value: string) => {
